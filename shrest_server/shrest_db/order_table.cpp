...
order_table::order_table( string order_id_, string customer_id_, string product_id_, string order_amount_, string order_date_, string order_status_):
	SqlAccessor(),
	order_id(order_id_),
...
void order_table::add_order_table(){
	...
	string sql = "INSERT INTO 'order'( order_id, customer_id, product_id, order_amount, order_date, order_status )"
		"VALUES( ";
...
int order_table::get_order_table_count(){
	string count_sql = "SELECT count(1) FROM order";
...
void order_table::get_order_instance(std::map<string, string> &order){
	...
	" FROM order  WHERE order_id = ";
		...
		order["order_amount"] = res->get_string(3);

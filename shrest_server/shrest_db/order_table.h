...
public:
	order_table();
	order_table( string order_id, string customer_id, string product_id, string order_amount, string order_date, string order_status);
	~order_table();
	...
	string get_order_amount(){return order_amount;}
	...
	void set_order_amount(string order_amount_){ order_amount = order_amount_; }
